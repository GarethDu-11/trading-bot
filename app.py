...
# Huấn luyện mô hình Machine Learning (giả định dữ liệu mẫu)
def train_price_pattern_model():
    # Dữ liệu mẫu giả định (thực tế cần dữ liệu lớn hơn)
    X = np.array([
        [0.02, 0.01, 0.015, 0.5],  # Double Top
        [0.01, 0.02, -0.01, -0.3], # Double Bottom
        [0.03, 0.02, 0.01, 0.7],   # Head and Shoulders
        # Thêm các mẫu khác nếu có dữ liệu thực tế
    ])
...
def detect_price_pattern(df):
    ...
    # Tính đặc trưng cho Machine Learning
    ...
    # Kiểm tra thêm bằng logic thủ công
    patterns = {
...
def get_prices():
    ...
    for coin in tracked_coins:
        try:
            ...
            prices[coin["symfrom flask import Flask, render_template, request, jsonify
import pandas as pd
import numpy as np
from tvDatafeed import TvDatafeed, Interval
import schedule
...
NEWS_API_KEY = "YOUR_NEWSAPI_KEY_HERE"  # Thay bằng key của bạn
