...
# Khởi tạo TvDatafeed với phương thức nologin
tv = TvDatafeed(username=None, password=None)

# Danh sách coin gốc bao gồm BTC, ETH, XRP, SOL, ALPH, KAS
default_coins = [
...
def add_coin():
    symbol = request.form.get('symbol').upper() + "USDT"
...
def get_prices():
    ...
    for coin in tracked_coins:
        try:
            ...
            prices[coin["symbol"]] = df['close'].iloc[-1] if not df.empty else None
        except Exception as e:
            prices[coin["symbol"]] = None
    return jsonify(prices)

# Endpoint lấy biến động 24h
@app.roufrom flask import Flask, render_template, request, jsonify
import pandas as pd
import numpy as np
from tvDatafeed import TvDatafeed, Interval
import schedule
...
if __name__ == '__main__':
    ...
    app.run(debug=True)te('/get_price_change_24h', methods=['GET'])
def get_price_change_24h():
    changes = {}
    for coin in tracked_coins:
        try:
...
if __name__ == '__main__':
    daily_analysis()  # Chạy phân tích lần đầu khi khởi động
    app.run(host='0.0.0.0', port=10000)  # Khớp với cấu hình Render